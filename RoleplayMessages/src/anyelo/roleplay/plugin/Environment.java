...
public class Environment implements CommandExecutor {

	private Main plugin;

	public Environment(Main plugin) {
		this.plugin = plugin;
	}
	...
	public boolean onCommand(CommandSender sender, Command command, String Iable, String[] args) {
		...
		if (!(sender instanceof Player)) {
			...
			if (args.length > 0
					&& player.hasPermission(plugin.getConfig().getString("permission.Environment_Permission"))) {
